...
from caipeex.utils import get_ss_full_inclrain_and_vent, get_nconc, \
                          get_meanfr
...
def main():
    ...
    for f in files:
        ...
        ss_qss = get_ss_full_inclrain_and_vent(dataset, metdata)
        meanfr = get_meanfr(dataset, metdata)
        ...
        ax.set_xlabel('SS (%)')
        ax.set_ylabel('Count')
        outfile = FIG_DIR + versionstr + 'ss_from_dsd_inclrain_and_vent_' \
                + date + '_figure.png'
        plt.savefig(outfile)
        plt.close(fig=fig)
