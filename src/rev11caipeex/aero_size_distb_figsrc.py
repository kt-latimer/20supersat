"""
make and save histograms showing aerosol nconc distribution from 2011 HALO PCASP measurements
"""
import csv
...
from rev11caipeex import DATA_DIR, FIG_DIR, PCASP_bins
...
n_samples = -1 #v3; filter out all met data times 
...
def main():
    ...
    for date in good_dates:
        ...
        lwc = met_dict['data']['lwc_from_file']
        temp = met_dict['data']['temp']
        
        time_met = met_dict['data']['time']
        time_pcasp = pcasp_dict['data']['time']

        #time_sync_inds = [t in time_met for t in time_pcasp]
        #
        #filter_inds = np.logical_and.reduce(( \
        #                                (temp > 273), \
        #                                (lwc < lwc_filter_val)))
        
        time_sync_inds = [True for i in range(np.shape(time_pcasp)[0])]
        filter_inds = [t not in time_met for t in time_pcasp]

        print(np.sum(filter_inds))
        print(np.shape(filter_inds))

        aero_size_distb = get_aero_size_distb(pcasp_dict['data'], \
                                                filter_inds, time_sync_inds,\
                                                n_samples)
...
def get_aero_size_distb(pcasp_data, filter_inds, time_sync_inds, n_samples):
    ...
    for i in range(n_bins):
        ...
        dN = np.array([np.nanmean( \
                pcasp_data[var_key][time_sync_inds][filter_inds][j:j+n_samples]) \
