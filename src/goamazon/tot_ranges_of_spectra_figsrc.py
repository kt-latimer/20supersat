...
def main():
    ...
    for data_date_tuple in data_date_tuples:
        ...
        tot_nconc = get_tot_nconc(smpsvars)

        pcen0 = np.percentile(tot_nconc, 0, interpolation='nearest')
        pcen25 = np.percentile(tot_nconc, 25, interpolation='nearest')
        pcen50 = np.percentile(tot_nconc, 50, interpolation='nearest')
        pcen75 = np.percentile(tot_nconc, 75, interpolation='nearest')
        pcen100 = np.percentile(tot_nconc, 100, interpolation='nearest')

        pcen0_ind = np.where(tot_nconc == pcen0)[0][0]
        pcen25_ind = np.where(tot_nconc == pcen25)[0][0]
        pcen50_ind = np.where(tot_nconc == pcen50)[0][0]
        pcen75_ind = np.where(tot_nconc == pcen75)[0][0]
        pcen100_ind = np.where(tot_nconc == pcen100)[0][0]
...
def get_tot_nconc(smpsvars):

    tot_nconc = np.sum( \
            smps_dlogDp*smpsvars['number_size_distribution'][...], \
            axis=1)
...
def make_and_save_range_of_spectra_plot(pcen0, pcen0_ind, \
    ...
    ax.set_title(date + ' range of aerosol size distributions (by tot nconc)')
    ax.legend()
    outfile = FIG_DIR + versionstr + 'tot_ranges_of_spectra_' \
            + date + '_figure.png'
