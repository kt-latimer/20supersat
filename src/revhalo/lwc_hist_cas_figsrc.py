...
def main():
    ...
    for date in good_dates:
        ...
        make_and_save_lwc_hist(lwc, date, versionstr, change_cas_corr, \
...
def make_and_save_lwc_hist(lwc, label, versionstr, change_cas_corr, \
                                cutoff_bins, full_ss, incl_rain, incl_vent):
    ...
    ax.set_title(label + ' LWC distb' \
                    ...
                    + ', full_ss=' + str(full_ss))
    outfile = FIG_DIR + versionstr + 'lwc_hist_cas_' \
            + label + '_figure.png'
    plt.savefig(outfile)
    plt.close(fig=fig)    
