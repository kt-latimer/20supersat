...
def main():
    ...
    for date in good_dates:
        ...
        make_and_save_z_hist(z, date, versionstr, change_cas_corr, \
                                    cutoff_bins, full_ss, incl_rain, incl_vent)

    print(z_alldates)
    make_and_save_z_hist(z_alldates, 'alldates', versionstr, \
            change_cas_corr, cutoff_bins, full_ss, incl_rain, incl_vent)

def add_to_alldates_array(z, z_alldates):

    if z_alldates is None:
        return z 
    else:
        return np.concatenate((z_alldates, z))

def make_and_save_z_hist(z, label, versionstr, change_cas_corr, \
                                cutoff_bins, full_ss, incl_rain, incl_vent):
    
    fig, ax = plt.subplots()
    fig.set_size_inches(21, 12)
    ax.hist(z, bins=30, density=False)
    ax.set_xlabel('z (m)')
    ax.set_ylabel('Count')
    ax.set_title(label + ' altitude distb' \
                    + ', change_cas_corr=' + str(change_cas_corr) \
                    + ', cutoff_bins=' + str(cutoff_bins) \
                    + ', incl_rain=' + str(incl_rain) \
                    + ', incl_vent=' + str(incl_vent) \
                    + ', full_ss=' + str(full_ss))
    outfile = FIG_DIR + versionstr + 'z_hist_cas_' \
            + label + '_figure.png'
    plt.savefig(outfile)
    plt.close(fig=fig)    
