...
def get_z_bins(z):
    ...
    #lower and upper bins bounded on bottom and top by global min and top 
    #layer max val of z array (resp). Inner bin boundaries given by mean 
    #values of the grid layers.
    n_bins = np.shape(z)[1]
    n_edges = n_bins + 1
    ...
    for i in range(1, n_bins):
        ...
        if layer_geom_mean < z_max:
            ...
            break
    z_bins.append(np.max(z[:, n_bins-1, :, :]))
