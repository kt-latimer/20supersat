...
def main():
    ...
    filename ='filtering_criteria_mse_data.npy'
...
def get_filter_dependent_values(case_label, case_dir_name, \
    ...
    mse_arr = np.zeros((n_lwc_vals, n_w_vals))
    ...
    for i, j in product(range(n_lwc_vals), range(n_w_vals)):
        ...
        if np.sum(filter_inds) == 0:
            ...
            mse = np.sum((ss_qss_filt - ss_wrf_filt)**2.)/npts

            lh_sum_arr[i, j] = np.sum(lh_filt)
            mse_arr[i, j] = mse
            npts_arr[i, j] = npts

    case_data_dir = {'lh_sum_arr': lh_sum_arr, \
                    'lwc_cutoffs_arr': lwc_cutoffs_arr, 'mse_arr': mse_arr, \
                    'npts_arr': npts_arr, 'w_cutoffs_arr': w_cutoffs_arr}
