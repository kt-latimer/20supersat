...
def make_and_save_ss_qss_vs_ss_wrf(case_label, case_dir_name, \
    ...
    rain_rate = rain_rate[filter_inds]
    ...
    filename = 'rain_rate_data_' + case_label + '.npy'
    np.save(DATA_DIR + case_dir_name + filename, rain_rates)
    print(np.shape(rain_rates))

    im = ax.pcolormesh(ss_bins, ss_bins, rain_rates, cmap=plt.cm.magma_r)

    cb = fig.colorbar(im, ax=ax)
    cb.set_label(r'Rain rate (mm/s)')
    ...
    outfile = FIG_DIR + 'rainrate_heatmap_ss_qss_vs_ss_wrf_' \
                            + case_label + '_figure.png'
    plt.savefig(outfile)
    plt.close()    

def get_rain_rates(rain_rate, ss_bins, ss_qss, ss_wrf):

    n_bins = np.shape(ss_bins)[0] - 1
    rain_rates = np.zeros((n_bins, n_bins))
    ...
    for i, val1 in enumerate(ss_bins[:-1]):
        for j, val2 in enumerate(ss_bins[:-1]):
            ...
            if np.sum(bin_inds) > 0:
                rain_rate_filt = rain_rate[bin_inds]
                rain_rates[i, j] = np.mean(rain_rate_filt)
            else:
                rain_rates[i, j] = np.nan 
