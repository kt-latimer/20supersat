"""
Heatmap scatter plot showing agreement bt ss_qss and ss_wrf, varying values of
rmax used to calculate LWC (where LWC \equiv \sum_{r <= rmax}n(r)*4/3\pi r^3*rho_{h20,
liq}/rho_{air}). Print value of rmax in figure (used to create gif)
"""
import matplotlib
...
def main():
    ...
    for case_label in case_label_dict.keys():
        ...
        make_and_save_heatmap(case_label, case_label_dict[case_label], \
                    cutoff_bins, full_ss, incl_rain, incl_vent, imax)

def make_and_save_heatmap(case_label, case_dir_name, cutoff_bins, full_ss, \
                                                incl_rain, incl_vent, imax):
    ...
    rmax = str(WRF_bin_radii[imax]*1.e6)
    ax.text(0.7, 0.9, r'$r_{max}='+rmax+'\mu m$')
    fig.suptitle('Actual versus approximated supersaturation - WRF ' + case_label)

    outfile = FIG_DIR + 'heatmap_vary_rmax_ss_qss_vs_ss_wrf_' + \
                            str(imax) + case_label + '_figure.png'
    plt.savefig(outfile)
    plt.close()    
