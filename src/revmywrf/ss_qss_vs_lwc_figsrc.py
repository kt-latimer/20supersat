...
def main():
    ...
    for case_label in case_label_dict.keys():
        make_and_save_ss_qss_vs_lwc(case_label, case_label_dict[case_label], \
                                    cutoff_bins, full_ss, incl_rain, \
                                    incl_vent, versionstr)

def make_and_save_ss_qss_vs_lwc(case_label, case_dir_name, \
                                cutoff_bins, full_ss, \
    ...
    m, b, R, sig = linregress(ss_qss*100, lwc*100)
    print(m, b, R**2)
   
    print_point_count_per_quadrant(ss_qss, lwc)

    ax_lims = np.array([-100, 100])
    #plot the supersaturations against each other with regression line
    fig, ax = plt.subplots()
    ax.scatter(ss_qss, lwc, c=colors['ss'])
    ...
    ax.set_ylabel(r'LWC cloud (kg/kg)')
    fig.legend(loc=2)
    fig.set_size_inches(21, 12)
    ax.set_title(case_label + ' SS_QSS vs LWC cloud' \
                    + ', cutoff_bins=' + str(cutoff_bins) \
                    + ', incl_rain=' + str(incl_rain) \
                    + ', incl_vent=' + str(incl_vent) \
                    + ', full_ss=' + str(full_ss))
    outfile = FIG_DIR + versionstr + 'ss_qss_vs_lwc_' \
