...
def make_and_save_ss_qss_vs_z(case_label, case_dir_name, \
    ...
    m, b, R, sig = linregress(ss_qss, z)
    print(m, b, R**2)
   
    print_point_count_per_quadrant(ss_qss, z)

    ax_lims = np.array([-100, 100])
    #plot the supersaturations against each other with regression line
    fig, ax = plt.subplots()
    ax.scatter(ss_qss, z, c=colors['ss'])
    ...
    ax.set_title(case_label + ' SS_QSS vs altitude' \
                    + ', cutoff_bins=' + str(cutoff_bins) \
                    + ', incl_rain=' + str(incl_rain) \
                    + ', incl_vent=' + str(incl_vent) \
                    + ', full_ss=' + str(full_ss))
    outfile = FIG_DIR + versionstr + 'ss_qss_vs_z_' \
