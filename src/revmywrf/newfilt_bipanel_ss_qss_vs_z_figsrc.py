...
def get_ss_qss_and_z_data(case_label):
    ...
    print(case_label)
    print(np.nanmean(qvstar))
    print(np.nanmin(qvstar))
    print(np.nanmax(qvstar))
    print(np.nanstd(qvstar))
    return 0, 0, 0, 0, 0
    ...
    del lwc, qvstar, temp #for memory
...
def make_and_save_bipanel_ss_qss_vs_z(ss_qss_dict, z_dict, z_bins_dict, color, label):
    ...
    outfile = FIG_DIR + versionstr + 'newfilt_bipanel_ss_qss_vs_z_' \
            + label + '_figure.png'
    plt.savefig(outfile)
    plt.close(fig=fig)    
