...
from revmywrf.ss_qss_calculations import get_lwc, get_meanr, linregress
...
def main():
    ...
    for case_label in case_label_dict.keys():
        make_and_save_meanr_vs_z(case_label, case_label_dict[case_label], \
                                    cutoff_bins, full_ss, incl_rain, \
                                    incl_vent, versionstr)

def make_and_save_meanr_vs_z(case_label, case_dir_name, \
                                cutoff_bins, full_ss, \
    ...
    m, b, R, sig = linregress(meanr, z)
    print(m, b, R**2)
   
    print_point_count_per_quadrant(meanr, z)

    #plot the supersaturations against each other with regression line
    fig, ax = plt.subplots()
    ax.scatter(meanr, z, c=colors['ss'])
    ...
    ax.set_xlim(ax_lims)
    ax.set_xlabel(r'$\langle f(r)r \rangle$ (m)')
    ax.set_ylabel(r'z (m)')
    fig.legend(loc=2)
    fig.set_size_inches(21, 12)
    ax.set_title(case_label + ' mean drop radius vs altitude' \
                    + ', cutoff_bins=' + str(cutoff_bins) \
                    + ', incl_rain=' + str(incl_rain) \
                    + ', incl_vent=' + str(incl_vent) \
                    + ', full_ss=' + str(full_ss))
    outfile = FIG_DIR + versionstr + 'meanr_vs_z_' \
