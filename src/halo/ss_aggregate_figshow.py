...
def filter_and_rescale(datablock, change_cas_corr, cutoff_bins):
    ...
    T = T[filter_inds] 
    print('len(w)', np.shape(w)[0])
    print('len(w_filt)', np.sum(filter_inds))
    w = w[filter_inds]
...
def make_comparison_scatter(datablock_control, datablock_modified,
    ...
    (ss_cas_modified, ss_cdp_modified) = get_ss_vs_t(meanr_cas_modified, \
                                    ...
                                    nconc_cdp_modified, T_modified, w_modified)
    print(np.shape(ss_cas_modified))
    print(np.sum(ss_cas_modified > 2))
    print(np.sum(ss_cdp_modified > 2))
    print(np.sum(np.logical_and.reduce(( \
            (ss_cas_modified > 2), \
            (ss_cdp_modified > 2)))))
