...
#for plotting
colors = {'abs_w': '#BA3F00', 'ss': '#88720A', 'tas': '#095793'}
versionstr = 'v3_'
...
def main():
    ...
    #filename = DATA_DIR + 'ss_optimal_offsets.txt'
    filename = DATA_DIR + 'ss_optimal_offsets_std_99prctl.txt'
    ...
    #lay out plot
    fig, ax = plt.subplots(4, 3, sharex=True, sharey=False, figsize=(16,16))
    ...
    for i in range(int(len(lines)/lines_per_date)):
        ...
        ss_std = [float(line[1]) for line in corr_data]
        ss_99p = [float(line[2]) for line in corr_data]

        ax[rows[i]][cols[i]].plot(offsets, ss_99p, \
                                    color=colors['abs_w'], \
                                    linestyle='-', \
                                    marker='o')
        ax[rows[i]][cols[i]].set_title(datestr)
        #ax[rows[i]][cols[i]].plot(offsets, ss_std, \
        #                            color=colors['tas'], \
        #                            linestyle='-', \
        #                            marker='o')
        #ax[rows[i]][cols[i]].set_title(datestr)
        #ax[rows[i]][cols[i]].plot(offsets, ss_rsq, \
        #                            color=colors['ss'], \
        #                            linestyle='-', \
        #                            marker='o',
        #                            label='QSS supersaturation')
        #ax[rows[i]][cols[i]].set_title(datestr)
    ...
    #plt.ylabel('SS Standard Deviation (%)')
    plt.ylabel('SS 99th percentile (%)')
    fig.set_size_inches(21, 15)
    #handles, labels = ax[3][2].get_legend_handles_labels()
    #plt.legend(handles, labels, loc='upper right')
    
    outfile = FIG_DIR + versionstr + 'ss_optimize_offsets_figure.png'
    plt.savefig(outfile)
