"""
Create and save meanr_vs_lwc figure set. 
"""
...
def main():
    ...
    scatter plot of LWC vs mean particle radius - one for CAS, one for CDP.
    ...
    for i, date in enumerate(dates):
        ...
        (meanr_cas, meanr_cdp) = get_meanr_vs_t(datablock_offset, change_cas_corr,
                                                cutoff_bins)
        meanr_cas = meanr_cas*1.e6
        meanr_cdp = meanr_cdp*1.e6
        ...
        make_figure(meanr_cas, meanr_cdp, lwc_cas, lwc_cdp, date)
        ...
        if i == 0:
            ...
            all_dates_meanr_cas = np.concatenate((
                                        all_dates_meanr_cas, meanr_cas))
            all_dates_meanr_cdp = np.concatenate((
                                        all_dates_meanr_cdp, meanr_cdp))
    ...
    make_figure(all_dates_meanr_cas, all_dates_meanr_cdp, all_dates_lwc_cas,
                all_dates_lwc_cdp, 'alldates')

    #blank figure for make compatibility
    fig, ax = plt.subplots()
    outfile = FIG_DIR + 'meanr_vs_lwc_set_figure.png'
    fig.savefig(outfile)

def make_figure(meanr_cas, meanr_cdp, lwc_cas, lwc_cdp, date):
    """
    Make scatter plot of meanr vs lwc for both instruments (same axis)
    """
    ...
    fig, ax = plt.subplots()
    ax.scatter(meanr_cas, lwc_cas, \
                    c=colors['CAS'], \
                    label='CAS')
    ax.scatter(meanr_cdp, lwc_cdp, \
                    c=colors['CDP'], \
                    label='CDP')
    ax.set_xlim(x_lims)
    ax.set_ylim(y_lims)
    ax.set_xlabel('Mean radius (um)')
    ...
    figtitle = 'Mean radius (um) vs LWC, Date: ' + date
    fig.suptitle(figtitle)
    outfile = FIG_DIR + versionstr + 'meanr_vs_lwc_' + date + '.png'
    fig.savefig(outfile)
    plt.close()
