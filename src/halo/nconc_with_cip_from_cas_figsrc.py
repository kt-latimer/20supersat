...
def main():
    ...
    for i, date in enumerate(dates):
        ...
        #        np.median(nconc[filter_inds]), np.min(nconc[filter_inds]), \
        #        np.max(nconc[filter_inds]))
        ...
        ax.hist(nconc, bins=30)#, color=colors['tot_derv'])
        ...
        ax.set_xlabel('nconc (s)')
        ax.set_ylabel('Count')
        outfile = FIG_DIR + versionstr + 'nconc_with_cip_from_cas_' \
                + date + '_figure.png'
        plt.savefig(outfile)
        plt.close(fig=fig)
