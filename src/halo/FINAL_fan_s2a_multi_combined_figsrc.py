...
def main():
    ...
    for i, method in enumerate(nconc_methods.keys()):
        ...
        fig, ax_arr = get_ax_fan_s2a_multi(nconc, wmax, m, b, R, \
                p_val, method, method_label, fig, ax_arr, ind0, ind1)

    fig.suptitle('Max vertical wind velocity versus aerosol number concentration')
    outfile = FIG_DIR + 'FINAL_fan_fig_s2a_multi_combined.png'
...
def get_ax_fan_s2a_multi(nconc, wmax, m, b, R, p_val, method, \
                            method_label, fig, ax_arr, ind0, ind1):
        ...
        ax.set_xlim([0, 6000])
        ax.set_ylim([0, 70])
        ax.text(0.05, 0.8, 'slope p value = ' + str(p_val), transform=ax.transAxes)
        ax.legend()

        ax.set_title('Number concentrations from ' + method_label)
