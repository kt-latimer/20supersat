...
def main():
    ...
    for i, date in enumerate(dates):
        ...
        #ax.set_title('tau distribution, w > 2 m/s')
        #ax.set_title('tau distribution, LWC > 1e-5 g/g, w > 2 m/s')
        ax.set_xlabel('tau (s)')
        ax.set_ylabel('Count')
        outfile = FIG_DIR + versionstr + 'tau_with_cip_from_cas_' \
                + date + '_figure.png'
        plt.savefig(outfile)
        plt.close(fig=fig)
    ...
    ax.set_title('tau distribution, LWC > 1.e-4 g/g, T > 273K, w > 2 m/s, \
        with raindrops and ventilation corrections')
    #ax.set_title('tau distribution, LWC > 1e-5 g/g')
    #ax.set_title('tau distribution, w > 2 m/s')
    #ax.set_title('tau distribution, LWC > 1e-5 g/g, w > 2 m/s')
    ax.set_xlabel('tau (s)')
    ax.set_ylabel('Count')
