...
def main():
    ...
    for splice_method in splice_methods[:1]:
...
def make_ss_hist_series(splice_method):
    ...
    make_ss_hist_from_rmax(rmax1, rmax2, splice_method)
...
def make_ss_hist_from_rmax(rmax1, rmax2, splice_method):
    ...
    for date in good_dates:
        print(date)
        ss_qss = get_ss_qss_data(date, rmax1, rmax2, splice_method)
...
def get_ss_qss_data(date, rmax1, rmax2, splice_method):
    ...
    lwc1 = get_lwc_with_rmax(adlr_dict, cas_dict, cip_dict, \
            change_cas_corr, False, rmax1, splice_method)
    lwc2 = get_lwc_with_rmax(adlr_dict, cas_dict, cip_dict, \
            change_cas_corr, False, rmax2, splice_method)
    ...
    for i, val in enumerate(filter_inds2):
        if val and not filter_inds1[i]:
            print(i, ss_qss[i], lwc1[i], lwc2[i])

    if np.sum(filter_inds1) != 0:
        ss_qss = ss_qss[filter_inds1]
    else:
        ss_qss = np.array([])
...
def make_and_save_ss_qss_hist(ss_qss, rmax, splice_method):
    ...
    ax.text(0.8, 0.8, r'$r_{max}$ = ' + str(rmax), \
