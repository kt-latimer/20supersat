    ...
    for i, date in enumerate(dates):
        ...
        for offset in range(-9, 9):
            ...
            filter_inds = np.logical_and.reduce((
                            ...
                            np.logical_not(np.isnan(tas_adlr)), \
                            np.logical_not(np.isnan(tas_cas))))

            #apply lwc filters
            tas_adlr = tas_adlr[filter_inds]
            tas_cas = tas_cas[filter_inds]

            #get linear regression params
            m_tas, b_tas, R_tas, sig_tas = linregress(tas_adlr, tas_cas)
            print(offset, R_tas**2, m_tas, b_tas)
