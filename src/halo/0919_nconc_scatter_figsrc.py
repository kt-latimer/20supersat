"""
Create and save figure 0919_nconc_scatter.
"""
...
def main():
    ...
    (cas_nconc, cdp_nconc) = get_nconc_vs_t(datablock)
    fig, ax = plt.subplots()
    ax.scatter(cas_nconc*1.e-6, cdp_nconc*1.e-6)
    coef = np.polyfit(cas_nconc, cdp_nconc, 1)
    print(coef)
    poly1d_fn = np.poly1d(coef) 
    ax.plot(cas_nconc, poly1d_fn(cas_nconc), '--k')
    ax.set_xlabel('CAS')
    ax.set_ylabel('CDP')
    ax.set_title('Total number concentration (cm^-3)')
    ax.set_aspect('equal', 'datalim')
    fig.set_size_inches(21, 12)
    outfile = FIG_DIR + '0919_nconc_scatter_figure.png'
    plt.savefig(outfile)
