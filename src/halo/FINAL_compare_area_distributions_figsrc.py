"""
Make plots comparing d(N*r^2)/dlogDp for HALO and both WRF simulations
"""
import matplotlib
...
y_labels_dict = {'with_vent': r'$\frac{d(r^2 \cdot f(r) ' + \
                    r'\cdot N(r))}{d\log r}$ ($\frac{\mu m^2}{cm^3}$)', \
                 'no_vent': r'$\frac{d(r^2 \cdot N(r))}' + \
                    r'{d\log r}$ ($\frac{\mu m^2}{cm^3}$)'}
...
def main():
    ...
    make_area_distribution_fig(spectrum_dict)
...
def make_area_distribution_fig(spectrum_dict):
    ...
    outfile = FIG_DIR + 'FINAL_compare_area_distributions_figure.png'
