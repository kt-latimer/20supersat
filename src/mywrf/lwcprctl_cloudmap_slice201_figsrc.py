...
def main():
    ...
    for model_label in model_dirs.keys():
        ...
        lwc_all = ncsecvars['lwc_cloud'][...]
        #ss_qss = ncsecvars['ss_qss'][...]
        #ss_wrf = ncsecvars['ss_wrf'][...]
        temp_all = ncsecvars['temp'][...]
        w_all = ncsecvars['w'][...]
        ...
        mask = np.logical_and.reduce(( \
                                    ...
                                    (temp_all > 273), \
                                    (w_all > 2)))
        bulk_cutoff = np.percentile(lwc_all[mask], 5)
        ...
        for t in range(84):
            ...
            cloud_mask = np.logical_and.reduce(( \
                                        ...
                                        (temp_t > 273), \
                                        (w_t > 2)))
            edge_mask = np.logical_and(cloud_mask, lwc_t < bulk_cutoff)
            cloudmap = np.ones(np.shape(cloud_mask))
            found_cloud_point = False
            found_edge_point = False
            for i, row in enumerate(cloudmap):
                for j, col in enumerate(row):
                    if edge_mask[i, j]:
                        found_edge_point = True
                        cloudmap[i, j] = 2
                    elif cloud_mask[i, j]:
                        found_cloud_point = True
                        cloudmap[i, j] = 0

            if True:#found_cloud_point and found_edge_point
                fig, ax = plt.subplots()
                ...
                outfile = FIG_DIR + versionstr + 'lwcprctl_cloudmap_slice201_' \
