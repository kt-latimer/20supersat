...
'lwc_tot' (lwc in cloud and rain; g/g), 'meanr' (mean cloud drop radius; m),
'nconc' (cloud drop number concentration; m^-3), 'pres' (total pressure; Pa),
'rho_air' (dry air density from ideal gas law; kg/m^3), 'ss_qss' (quasi-steady-
state supersaturation, no ventilation), 'ss_ry' (supersaturation from 
condensation rate as given in Rogers and Yau, no ventilation), 'ss_wrf' 
...
def main():
    ...
    for model_label in model_dirs.keys():
        ...
        ncoutfile = Dataset(DATA_DIR + model_dir \
                        + 'wrfout_d01_secondary_vars_no_rain_2', 'w')
