...
def main():
    ...
    for model_label in model_dirs.keys():
        ...
        lwc_cutoffs = [(1.+2.*i)*1.e-5 for i in range(20)]
        nq4 = []
        ntot = []
        for lwc_cutoff in lwc_cutoffs:
        ...
        #plot shit 
        fig, ax = plt.subplots()
        ax.scatter(lwc_cutoffs, np.array(nq4)/np.array(ntot)*100, c=colors['ss'])
        ax.set_xlabel(r'$LWC_{cutoff}$ (m/s)')
        ax.set_ylabel(r'$N_{Q4}/N_{tot}$ (%)')
        ax.set_xlim([np.min(lwc_cutoffs), np.max(lwc_cutoffs)])
        fig.set_size_inches(21, 12)

        outfile = FIG_DIR + versionstr + 'inclrain_and_vent_nq4_vs_lwc_cutoff_' \
